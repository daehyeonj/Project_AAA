...
- Batch 90 encounter-variety rule: Alpha Stability/Surge battle feel may be clarified in the battle request/context and popover by reading the selected route, authored encounter/battle setup, live monster HP/ATK/target-pattern totals, and active room-interaction consequence state. This is readback only; do not mutate enemy stats, add status effects, create new route-pressure state, or rewrite ResultPipeline to make the battles feel different.
- Batch 91 runtime proof/balance rule: Stability `Threat: Moderate` and Surge `Threat: High` labels are DungeonRun/BattleScene display projections over the same Batch90 data-backed route-pressure seam. The proof may compare those labels, ATK totals, predicted damage, Shrine Protection setup, Cache Pressure payoff, and popover consequences, but must not add new combat math or route state.
- Batch 92 dilemma-variation rule: Rest Shrine / Greed Cache may now resolve as either use or skip inside the same grid-dungeon route. Using keeps the existing `Shrine Protection` / `Cache Pressure` consequence path; moving past the prompted route-defining room records a DungeonRun-local skip consequence (`no Shrine Protection` or `no Cache Pressure` / no extra payout), advances the room sequence, and surfaces that honest result through the existing battle context, encounter popover, ResultPipeline event-choice field, and world board. Do not add a new choice shell, fake penalty, fatigue system, or ResultPipeline payload for the skip branch.
