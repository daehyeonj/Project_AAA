...
- Batch 47 draft-retarget guard: when readiness/preflight/context show a blocked draft on a saturated supported rail, also run `GoldenPathAuthoringDraftHelper.RunBatchTemplateDraftRetargetSummary` or `GoldenPathAuthoringDraftHelper.RunCreateBatchTemplateRetargetedDraft`. Treat `RetargetState=candidate:*hidden-canonical-retarget` as off-rail hidden-canonical staging proof, not as surfaced-promotion proof, and treat `RetargetCurrentRailState=blocked:requires-runtime-route-surface-expansion` as expected when the helper is intentionally stepping outside the current Alpha/Beta safe/risky surfaced seam.
- Batch 48 hidden-canonical promotion guard: when a retargeted draft is already owner-safe, run `GoldenPathAuthoringDraftHelper.RunBatchTemplateRetargetPromotionSummary` and `GoldenPathAuthoringDraftHelper.RunPromoteBatchTemplateRetargetedDraft`, then rerun the surfaced matrix summary. Treat `Status=WARN:canonical-but-not-surfaced` on the new off-rail route as expected proof for a hidden-canonical promotion batch, as long as the surfaced portfolio count stays `4` and no surfaced route regresses to fallback or consumer mismatch.
