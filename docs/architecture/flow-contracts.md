...
- Batch 90 encounter-variety guard: Alpha battle readbacks may distinguish Stability `Slime Front` from Surge `Goblin Pair Hall`, but the copy must derive from the confirmed route, authored encounter/battle setup, live monster HP/ATK/target-pattern state, and active `Shrine Protection` / `Cache Pressure` consequence text. Do not add new combat mechanics, elemental/status rules, fake pressure, or per-frame route-pressure rebuilds.
- Batch 91 encounter-variety proof/balance guard: the same Alpha Stability/Surge battle-feel seam should remain runtime-proofable through battle context, enemy intent, target preview, and encounter popover evidence. Threat labels such as `Moderate` / `High` are display projections over the existing live setup numbers and must not become hidden stat mutation, a new combat rule, or a second route-pressure state.
...
- Batch 90 route-pressure readback: `PrototypeBattleRequest`, battle context, enemy intent labels, and target preview text may append route-pressure clauses for the current Alpha encounter. These are display projections over existing DungeonRun and BattleScene state; `PrototypeBattleRuntimeState` remains the live battle contract and no enemy stat, skill, inventory, or result rule should be invented for the readback.
- Batch 91 runtime balance proof: targeted proof may compare Stability `Threat: Moderate` / lower ATK/predicted damage against Surge `Threat: High` / Cache Pressure payoff using the same request/runtime/UI/popover contracts. This is a validation lock on Batch90 behavior, not a new battle-scaling system.
