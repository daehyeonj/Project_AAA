...
- Batch 86 wait-cost guard: from second ExpeditionPrep, verify `Wait Cost`, `Pressure Clock`, `[T] Recover 1 Day`, world day advancement, readiness recovery, selected-city stock/need tick readback such as `stock -1`, and the `Next: launch now; waiting again risks shortage` recommendation. Use `Batch82RepeatCoreLoopProofRunner.RunBatch86WaitCostPressureClockProof`, keep Batch10 smoke green, and use `docs/runtime/batch86-wait-cost-pressure-clock-checklist.md` for manual feel QA.
...
- Batch 87 route-feel guard: for `city-a -> dungeon-alpha -> safe/risky`, confirm route cards, DungeonRun readback, encounter popover, and final/world board distinguish Stability sustain/lower strain from Surge pressure/higher payout and recovery strain. Use `Batch82RepeatCoreLoopProofRunner.RunBatch87DungeonRouteFeelProof`, keep Batch10 smoke green, run authoring validation if content assets changed, and use `docs/runtime/batch87-dungeon-route-feel-checklist.md` for manual feel QA.
- Batch 88 room-interaction guard: for `city-a -> dungeon-alpha`, confirm Rest Shrine / Greed Cache are explicit `[E]` room interactions, use existing recovery/cache data, update encounter/result/world readbacks, and remain blocked by modal/battle gates. Use `Batch82RepeatCoreLoopProofRunner.RunBatch88DungeonRoomInteractionProof`, keep Batch10 smoke green, and use `docs/runtime/batch88-dungeon-room-interaction-checklist.md` for manual feel QA.
- Batch 89 next-beat consequence guard: for `city-a -> dungeon-alpha`, confirm Rest Shrine / Greed Cache interactions arm a real next-encounter readback (`Shrine Protection` / `Cache Pressure`), appear in battle context, resolve in the encounter popover, and preserve final/world consequence text without new combat/fatigue/result systems. Use `Batch82RepeatCoreLoopProofRunner.RunBatch89RoomInteractionConsequenceChainProof`, keep Batch10 smoke green, and use `docs/runtime/batch89-room-interaction-consequence-chain-checklist.md` for manual feel QA.
- Batch 90 encounter-variety guard: for `city-a -> dungeon-alpha`, compare Stability `Slime Front` against Surge `Goblin Pair Hall` after Greed Cache. Confirm battle context, enemy intent, target preview/payoff, encounter popover, result, and world board readbacks derive from route setup, live enemy HP/ATK/target-pattern state, and active `Cache Pressure` / `Rest Shrine` rails. Use `Batch82RepeatCoreLoopProofRunner.RunBatch90EncounterVarietyRoutePressureProof`, keep Batch10 smoke green, and use `docs/runtime/batch90-encounter-variety-route-pressure-checklist.md` for manual feel QA.
- Batch 91 encounter-variety runtime proof/balance guard: after Batch90, lock the same Alpha route-specific battle feel by proving Stability reads as `Threat: Moderate` with lower ATK/predicted damage and Shrine Protection setup, while Surge reads as `Threat: High` with Cache Pressure payoff, stronger predicted damage, and recovery strain. Use `Batch82RepeatCoreLoopProofRunner.RunBatch91EncounterVarietyRuntimeProofBalance`, keep Batch10 smoke green, and use `docs/runtime/batch91-encounter-variety-runtime-qa-checklist.md` for manual feel QA.
- Batch 92 dungeon-dilemma variation guard: for `city-a -> dungeon-alpha`, prove Rest Shrine and Greed Cache now support honest use-vs-skip branches in the current grid dungeon. Rest Shrine use must arm `Shrine Protection`, while its skip source must say no protection; Greed Cache skip must leave +3 mana_shard behind, avoid `Cache Pressure`, carry that into Goblin Pair Hall battle context/popover/result/world board, and not break the existing Greed Cache open path. Use `Batch82RepeatCoreLoopProofRunner.RunBatch92DungeonDilemmaVariationProof`, keep Batch10 smoke green, run Batch89 as the open-path regression, and use `docs/runtime/batch92-dungeon-dilemma-variation-checklist.md` for manual feel QA.
...
- Batch 90 route-pressure guard: when battle UI copy explains Stability/Surge encounter feel, verify the same surface still exposes canonical request/runtime/resolution data and that route-pressure text is not backed by hidden stat mutation, new skills, new fatigue, or a second ResultPipeline path.
- Batch 91 runtime balance guard: targeted proof should capture battle context, enemy intent, target preview, and popover evidence for both Alpha route identities before expanding dungeon dilemmas in Batch92. Treat missing proof of `Moderate` vs `High` route pressure as a readback/balance-lock regression, not as permission to add a new combat system.
...
- Batch 88 interaction packaging guard: room-interaction consequence copy should travel through existing `SelectedEventChoiceText` / `EventChoiceSummaryText` fields. Do not add a new ResultPipeline payload just to display Rest Shrine or Greed Cache outcomes.
...
- Batch 86 wait-cost handoff: the post-wait `stock -1` / pressure-clock copy should be read from selected-city economy day results and recovery/readiness state. If the underlying day tick is stable or shortage-only, the readback should say that honestly instead of inventing a penalty.
